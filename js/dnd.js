...
(function () {
  ...
  var limits = {
    top: pinsBlock.offsetTop + 130,
    right: pinsBlock.offsetWidth + pinsBlock.offsetLeft - mainPin.offsetWidth + 110,
    bottom: pinsBlock.offsetHeight + pinsBlock.offsetTop - mainPin.offsetHeight + -10,
  ...
  mainPin.addEventListener('mousedown', function (e) {
    e.preventDefault();
    isDragged = true;
    inputAddress.value = (e.clientX + ', ' + e.clientY);
  });

  pinsBlock.addEventListener('mouseup', function () {
    isDragged = false;
  });

  pinsBlock.addEventListener('mousemove', function (e) {
    e.preventDefault();
    if (isDragged) {
      move(e);
    }
